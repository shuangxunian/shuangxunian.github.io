...
浏览器会把inline内联元素间的空白字符（空格、换行、Tab等）渲染成一个空格。为了美观，通常是一个`li`放在一行，这导致换行后产生换行字符，它变成一个空格，占用了一个字符的宽度。
...
4. 消除`li`的字符间隔letter-spacing:-8px，不足：这也设置了`li`内的字符间隔，因此需要将`li`内的字符间隔设为默认letter-spacing:normal。
...
作者：Spirited_Away
链接：https://juejin.cn/post/7006521586836570126
来源：稀土掘金
...
这样就能缩放到原来的0.5倍，如果是1px那么就会变成0.5px。viewport只针对于移动端，只在移动端上才能看到效果；实际开发中，产品要是提，直接打回！
