...
## 前言
不理解没关系，看[这个](https://www.bilibili.com/video/BV1K24y1q7eJ/?spm_id_from=333.999.0.0&vd_source=35c2b2b5ca9b4285e152b8af4eb04974)
...
这种模式实现了 Model和View的数据自动同步，因此开发者只需要专注于数据的维护操作即可，而不需要自己操作DOM；即现在的vue等都是MVVM。
...
通过 router 的 Link 组件的 prop —— to 可以实现路由间传递参数的效果。
...
| 属性名称              | 合并策略                                                                                       | 对应合并函数       |
|-----------------------|----------------------------------------------------------------------------------------------|------------------|
| **data**             | mixins/extends 只会将自己有的但是组件上没有的内容混合到组件上，重复定义默认使用组件上的。<br>如果data里的值是对象，将递归内部对象继续按照该策略合并 | mergeDataOrFn, mergeData |
| **provide**          | 同上                                                                                          | mergeDataOrFn, mergeData |
| **props**            | mixins/extends 只会将自己有的但是组件上没有的内容混合到组件上                                 | extend           |
| **methods**          | 同上                                                                                          | extend           |
| **inject**           | 同上                                                                                          | extend           |
| **computed**         | 同上                                                                                          | extend           |
...
| **watch**            | 合并watch监控的回调方法<br>执行顺序是先mixins/extends里的watch定义的回调，然后是组件的回调          | strats.watch     |
| **HOOKS生命周期钩子**  | 同一种钩子的回调函数被合并成数组<br>执行顺序是先mixins/extends里的钩子函数，然后才是组件里定义的 | mergeHook        |
