...
## 前言
- 通过同一代码风格、文件格式编写vue项目会让合作以及接手的人可以以一个极短的时间内上手代码。
- 关于UI选择，
  - vue2.x优先使用elementUI，如果想使用其他UI请先和同事进行沟通，沟通一致后即可采用。
  - 关于vue3.0，此文写于21年3月22日，vue3.0还没有一套不错的UI，请留意后期更新。
- 关于js，
  - vue2.x中优先考虑es6，向下使用js。
  - vue3.0中优先考虑ts。
- 如果是一人从头开发一个小型项目，且可以在指定日期前独立完成，且不考虑后期维护的前提下，FEer可以考虑不采用以上编码规则，在保证性能的前提下使用一种自己最合适最快的即可。
- 推荐一款格式配置
...
- 事件移除时需要移除事件中心对应事件注明。
- 组件当中有原生绑定事件, 注意在组件注销前进行移除。


## 规则要求
以下内容整理自vue官方文档，仅仅做了略微改动，如果你已经很熟悉vue官方要求，以下内容可以省略
...
你可能想换成多级目录的方式，把所有的搜索组件放到“search”目录，把所有的设置组件放到“settings”目录。只推荐在非常大型 (如有 100+ 个组件) 的应用下才考虑这么做，因为：
